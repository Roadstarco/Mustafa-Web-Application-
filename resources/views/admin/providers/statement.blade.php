        ...
        <div class="container-fluid">
            <div class="box box-block bg-white">
            	...
            	<div style="text-align: center;padding: 20px;color: blue;font-size: 24px;">
            		<p><strong>
            			<span>Over All Earning : {{currency($revenue[0]->overall + $international_revenue[0]->overall)}}</span>
            			<br>
            			<span>Over All Commission : {{currency($revenue[0]->commission + $international_revenue[0]->commission )}}</span>
            		</strong></p>
            	</div>
	            	...
	            	<div class="col-lg-4 col-md-6 col-xs-12">
						<div class="box box-block bg-white tile tile-1 mb-2">
							...
							<div class="t-content">
								...
								<h1 class="mb-1">{{$rides->count() + $international_rides->count()}}</h1>
					...
					<div class="col-lg-4 col-md-6 col-xs-12">
						<div class="box box-block bg-white tile tile-1 mb-2">
							...
							<div class="t-content">
								...
								<h1 class="mb-1">{{currency($revenue[0]->overall + $international_revenue[0]->overall)}}</h1>
								<i class="fa fa-caret-up text-success mr-0-5"></i><span>from {{$rides->count() + $international_rides->count()}} Rides</span>
							</div>
						</div>
					</div>
						...
						<div class="row row-md mb-2" style="padding: 15px;">
							<div class="col-md-12">
									<div class="box bg-white">
										...
										@if(count($rides) != 0 || count($international_rides) != 0)
								            <table class="table table-striped table-bordered dataTable" id="table-2">
								                ...
								                <?php $diff = ['-success','-info','-warning','-danger']; ?>
                                                     <!--  Local Trips Start   -->
                                                        ...
                                                        <!--  International Trips Start   -->


                                                            @foreach($international_rides as $index => $ride)
															<tr>
																<td>{{$ride->booking_id}}</td>
																<td>
																	@if($ride->tripfrom != '')
																...
																<td>
																	@if($ride->status != "CANCELLED")
																		<a class="text-primary" href="{{'trips/show/'.$ride->id}}"><span                                                                                        class="underline">View Ride Details</span></a>
																	@else
																		<span>No Details Found </span>
																	@endif									
																</td>
																...
																<td>
																	@if($ride->trip_status == "COMPLETED")
																		<span class="tag tag-success">{{$ride->trip_status}}</span>
																	@elseif($ride->trip_status == "CANCELLED")
																		<span class="tag tag-danger">{{$ride->trip_status}}</span>
																	@else
																		<span class="tag tag-info">{{$ride->trip_status}}</span>
																	@endif
																</td>
