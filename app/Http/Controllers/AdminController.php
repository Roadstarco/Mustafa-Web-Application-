...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        try{
            ...
            /*******International Statistics added by nabeel hassan*******/

            $international_rides=Trip::has('user')->orderBy('id','desc')->get();
            $international_scheduled_rides=Trip::where('status','SCHEDULED')->count();
            $international_revenue=TripPayments::sum('total');


            return view('admin.dashboard',compact('providers','fleet','scheduled_rides','service','rides','user_cancelled','provider_cancelled','cancel_rides','revenue', 'international_rides', 'international_scheduled_rides', 'international_revenue'));
        }
        catch(Exception $e){
    ...
    public function settings_store(Request $request)
    {
        ...
        Setting::set('facebook_page_link', $request->facebook_page_link);
        Setting::set('instagram_page_link', $request->instagram_page_link);
        // Color Scheme
        Setting::set('site_color', $request->site_color);
        Setting::set('site_hover_color', $request->site_hover_color);
        Setting::set('marine_traffic_svp_api_key', $request->marine_traffic_svp_api_key);
        Setting::set('marine_traffic_eta_api_key', $request->marine_traffic_eta_api_key);
        Setting::set('flightaware_api_key', $request->flightaware_api_key);
        Setting::save();
    ...
    public function terms_and_conditions(){
        return view('admin.pages.static')
            ->with('title',"Page")
            ->with('page', "terms_and_conditions");
    }
    ...
    public function pages(Request $request){
        ...
        $this->validate($request, [
            'page' => 'required|in:page_privacy,terms_and_conditions',
            'content' => 'required',
        ]);
    ...
    public function statement($type = 'individual'){
        ...
        try{
            ...
            /**********International Rides Work By Nabeel Hassan*************/
            $international_rides = Trip::with('payment')->orderBy('id','desc');
            $international_cancel_rides = TripRequests::where('status','Rejected');
            $international_revenue = TripPayments::select(\DB::raw(
                'SUM(ROUND(fixed)) as overall, SUM(ROUND(commision)) as commission'
            ));
            ...
            if($type == 'today'){
                ...
                $international_rides->where('created_at', '>=', Carbon::today());
                $international_cancel_rides->where('created_at', '>=', Carbon::today());
                $international_revenue->where('created_at', '>=', Carbon::today());
                ...
                $revenue->where('created_at', '>=', Carbon::now()->month);
                  ...
                  $international_rides->where('created_at', '>=', Carbon::now()->month);
                $international_cancel_rides->where('created_at', '>=', Carbon::now()->month);
                $international_revenue->where('created_at', '>=', Carbon::now()->month);
                ...
                $international_rides->where('created_at', '>=', Carbon::now()->year);
                $international_cancel_rides->where('created_at', '>=', Carbon::now()->year);
                $international_revenue->where('created_at', '>=', Carbon::now()->year);
            ...
            $international_rides = $international_rides->get();
            $international_cancel_rides = $international_cancel_rides->count();
            $international_revenue = $international_revenue->get();


            return view('admin.providers.statement', compact('rides','cancel_rides','revenue' ,'international_rides', 
            'international_cancel_rides', 'international_revenue'))->with('page',$page);
...
//                }
                ...
                foreach ($Users as $User){
                    $tokenList[] = $User->device_token;
                }
                $this->sendNotification($tokenList, $title, $Push->message);
...
//                }
                ...
                $Providers = ProviderDevice::all();
                $title = "Caby Driver";
                foreach ($Providers as $Provider){
                    $tokenList[] = $Provider->token;
                }
                $this->sendNotification($tokenList,$title,  $Push->message);
    ...
    private function sendNotification($senders,$title, $message){

        define('API_ACCESS_KEY','AAAAGTwWSDk:APA91bF8VvY24ySZJdDTmn44Pg1KWei1S34cEekja_rSgGI8Sgd9oKosamE-W2kf_9rSs_4NHLqWrcqaJJm3GwWOyNHy0nAU3Xzfytna_VHjgjJQhUwxqTGYj6s_gbrDN184SBNa-HFo3l6aVEgCH81l17ZyY0vb_Q');
        ...
        $fcmNotification = [
            'registration_ids' => $senders, //multple token array
